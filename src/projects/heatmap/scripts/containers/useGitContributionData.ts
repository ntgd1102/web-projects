...
export const processGitContributionJson = (
    gitContributionJson?: WeeklyGitContribution[],
) => {
  if (!gitContributionJson) {
    return null;
  }
  const maxDailyContribution = gitContributionJson.reduce(
      (previousMaxDailyContribution, currentDailyContribution) => {
        const maxNumOfWeek = currentDailyContribution.days.reduce(
            (previousMax, curVal) => {
              return previousMax > curVal ? previousMax : curVal;
            },
            0,
        );
        return maxNumOfWeek > previousMaxDailyContribution ?
        maxNumOfWeek :
        previousMaxDailyContribution;
      },
      0,
  );
