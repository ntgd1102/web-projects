...
export const HeatMap: React.FC = () => {
    ...
    if (!result?.processedWeeklyGitContribution || !result?.processedWeeklyGitContribution.length) {
        return <div>HeatMap</div>
    }
    return (
        <div className='heat-map'>
            {
                result.processedWeeklyGitContribution.map((weeklyGitContribution: WeeklyGitContribution) => {

                    return (
                        <div className='weekly-git-contribution-container'>{
                            weeklyGitContribution.days.map((dailyContribution: number) => {
                                const className = getClassName(result?.maxDailyContribution, dailyContribution);
